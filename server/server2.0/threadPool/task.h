...
public:
    using Task = std::function<void()>; // 一个任务就是一个函数对象
...
// 添加任务时，先将一个函数变为无参函数（利用bind),
//然后利用package_task去掉其返回值（返回值类型被包装进package_task的future中），
//将一个匿名函数对象放入任务队列中，这个匿名函数内调用task函数
