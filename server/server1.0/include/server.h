...
private:
    ...
    std::shared_ptr<user> get_user_from_socketfd(int socketfd); //找到已连接的套接字对应的user对象，可能登录了也可能未登录 
    ...
    std::unordered_map<int,std::shared_ptr<user>> login_user; 
    //已连接但未登录用户
    std::mutex unlogin_user_mutex;
    std::unordered_map<int,std::shared_ptr<user>> unlogin_user; 
