...
std::unique_ptr<retmsg> taskpackage::login_handle(std::unique_ptr<abstractmsg> &&msg) // done 登录实现
{
    ...
    std::shared_ptr<sql::ResultSet> res = pool->executestatement("SELECT * FROM users");
    ...
    if (login_flag)
    {
        ...
        //登录成功后，还要查看username-msg的map容器内是否有发送给自己的消息
    }
    else
    {
        // 登录失败
...
std::unique_ptr<retmsg> taskpackage::register_handle(std::unique_ptr<abstractmsg> &&msg) // done
{
    ...
    if (register_flag)
    {
        ...
        pool->executestatement(insert_sql);
    }
    else
    {
        // 注册失败
...
{
    //todo chat消息的处理，要生成两个消息任务，一个发送给sender，一个发送给receiver 
    ...
    std::string sendmsg = chat_msg->get_msg();

    bool username_exist = false;
    std::string usernamequery = "SELECT * FROM users WHERE username = '" + recviver_name + "';";
    std::shared_ptr<sql::ResultSet> res = pool->executestatement(usernamequery);
    while (res->next())
    {
        username_exist = true;
    }
    ...
    else
    {
        ...
        {
            // 没上线
            // 存储在一个username-msg的map中,等用户上线了发送给他
            //生成返回sender的消息，告诉他消息发送成功但用户未上线
        ...
        {
            // 上线了，直接发送
            //生成返回sender的消息，告诉他消息发送成功  生成receiver的消息，向他发送消息
            // retmessage->set_msg("send success"); // 需要以json格式返回
            // retmessage->set_target_fd(chat_msg->get_receive_fd());
        }
    }
    // update 先到数据库中找到username，如果没有则发送失败
    // 如果有，则找username对应的fd，如果有则发送成功
    // 如果没有，则说明该用户未上线，则将消息存储在一个username-msg的map中
    // 等用户上线后，查看是否有该用户的消息，如果有则发送
