...
void Server::init()
{
    ...
    task->init();
    std::function<void(const std::string&,int)> temp1 = bind(&Server::add_login_user, this, std::placeholders::_1, std::placeholders::_2);
    task->set_add_loginuser_from_server(temp1);

    std::function<bool(const std::string &)> temp2 = bind(&Server::is_login, this, std::placeholders::_1);
    task->set_is_login_from_server(temp2);

    std::function<int(const std::string &)> temp3 = bind(&Server::get_socketfd_from_username, this, std::placeholders::_1);
    // m_analyser->set_get_fd_by_username_func(temp1);
    task->set_get_fd_by_username(temp3);

    // std::function<int(const std::string &)> temp4 = bind(&Server::get_socketfd_from_username, this, std::placeholders::_1);
    // task->set_get_fd_by_username(temp);

    std::function<void(std::string &, int)> temp4 = bind(&Server::send_msg, this, std::placeholders::_1, std::placeholders::_2);
    task->set_send_msg_in_task(temp4);
...
std::shared_ptr<user> Server::get_user_from_socketfd(int socketfd)
{
...
void Server::add_login_user(const std::string & username,int socketfd)
{
    ...
    std::shared_ptr<user> temp_user = it->second;
    unlogin_user.erase(it);
    unlogin_user_mutex.unlock();

    std::lock_guard<std::mutex> login(login_user_mutex);
    temp_user->set_username(username);
    login_user.insert({socketfd, temp_user});
...
void Server::read_msg_generate_task()
{
    while (true)
    {
        ...
        else
        {
            ...
            std::shared_ptr<user> temp_user = get_user_from_socketfd(socketfd); //todo
            if(temp_user == NULL)
            {
                continue; 
            }
