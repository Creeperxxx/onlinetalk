...
### 4. 关于套接字对应的存储收发数据的无锁队列中元素使用uint8_t还是vector<uint8_t>的问题
- 问题描述：为每个socket设计了两个无锁队列，存放收发数据。现在的问题是：无锁队列里面是直接存放所有消息的二进制，还是用vector将各个消息分割，也就是用一个vector存放一个消息的二进制？
    
- 个人认为：接收队列采用uint8_t，发送队列采用vector<uint8_t>

- 原因：
    - 接收队列：接收数据时可能产生延迟，丢包，缓冲区满等问题，也就是说可能无法立即得到整个消息并入队，如果使用vector<uint8_t>,那么岂不是到等到整个消息的二进制数据全部到达才能封装成一个vector<uint8_t>入队？那么就选择二进制方法。即使只部分消息，那也可以入队而不阻塞线程，然后延后处理这个不完整消息，就是先处理完整消息，在等到

### 5. 粘包半包问题，不对，应该说消息完整性判断问题
- 问题描述：将一个套接字读缓冲区的数据全部读取出来后，如何判断哪部分数据流为一个完整的消息？如何判断这个部分的数据流是否正确，也就是要校验？如果这个消息不完整，如何将其剔除掉，如何重发消息？
